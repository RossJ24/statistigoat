...
*In the boxplots shown above, the IQRS are all roughly the same for all eras. However, there are a large amount of outliers of the boxplot. Since our purpose is to isolate a singular player as the Greatest of all time, it might be best to consider only the players that are at the top.*
...
=======
>>>>>>> 9d04f371bbb3c8036f4113eeae4ae947a2f0fbdc
```
*fail to reject the null that the difference in mean GRs of BOS and LAL is zero with a alpha of 0.05. conclusion: there is no significant difference therefore the teams are the same level of greatness.*
