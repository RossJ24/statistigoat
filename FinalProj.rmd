...
alleras <- rbind(era1, era2, era3, era4, era5, era6)
...
alleras <- alleras[, c(3, 60, 61)]
alleras <- alleras %>% 
  group_by(Player) %>% 
  summarise(aGR = mean(GR, na.rm = TRUE), era = round(mean(Era, na.rm = TRUE), digits = 0))
...
```{r}
#to put to rest: should era factor into who the GOAT is? does it impact it? pair with ANOVA to answer this 
...
=======
>>>>>>> 154b5b2dd9e4dd0dc9f6c87170e6620807255dd1
...
#get version of szn_statsnb that only has complete cases. reorder so that GR is forst column
...
#some evidence of multicolinearity between the variables present. 
```
...
```{r}
library(leaps)
#use best subsets to find best model by BIC 

mod1 <- regsubsets(aGR ~ ., data = szn_statsnb2, nvmax = 8)
