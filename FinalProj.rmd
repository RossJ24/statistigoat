...
get_player_avgs <- function(eradf){
  ret <- eradf %>% 
  group_by(Player) %>% 
  summarise(aPPG = mean(PPG, na.rm = T), aAPG = mean(APG, na.rm = T), aORPG = mean(ORPG, na.rm = T), aDRPG = mean(DRPG, na.rm = T), aBPG = mean(BPG, na.rm = T), aTPG = mean(TPG, na.rm = T), aTSP = mean(TS., na.rm = T))
  return(ret)
}

get_era_with_player_name <- function(eradf){
  rowidx <- eradf$X
  tmp <- eradf
  tmp$Player <- szn_stats$Player[rowidx]
  return(tmp)
}

era1pavgs <- get_player_avgs(get_era_with_player_name(era1))
era2pavgs <- get_player_avgs(get_era_with_player_name(era2))
era3pavgs <- get_player_avgs(get_era_with_player_name(era3))
era4pavgs <- get_player_avgs(get_era_with_player_name(era4))
era5pavgs <- get_player_avgs(get_era_with_player_name(era5))
era6pavgs <- get_player_avgs(get_era_with_player_name(era6))
