...
We got our data from a kaggle dataset [here](https://www.kaggle.com/drgilermo/nba-players-stats?select=Seasons_Stats.csv) and downloaded it. The data we used was the basketball game statistics of all players from seasons 1950 -> 2017. This data was not game specific, but more of a total over the course of a season. This led to us having to divide each cumulative statistic by the number that particular player played that season resulting per game statistics. We chose to write a formula to represent a particular rating for a player's Goat likelihood. We called this statistic their Goat Rating (GR). We took into account statistics that have an obvious impact on the performance of a team, such as points per game (PPG), offensive rebounds per game (ORPG), defensive rebounds (DRPG), assists per game (APG), blocks per game (BPG), turnovers per game (TPG), true shooting percentage (TS.), and player efficiency rating (PER). *formula: GR = 2(PPG) + ⅘(DRPG) + ORPG + 1.5(APG) + PER - 1.5(TO) + Blocks + 10(TS%)*
...
*All variables are continuous, we have no categorical variables*
PPG- Points Per Game
...
A critical point of major disagreement during GOAT debates is whether superstars in older eras played against easier competitions. Some people say older eras had better defense, so it was harder to play back then. Others counter with the superior offense of more recent eras. We can use boxplots and One-way ANOVA to answer this question.

```{r}
# Make boxplots of GR for each era
boxplot(era1pavgs$aGR, era2pavgs$aGR, era3pavgs$aGR, era4pavgs$aGR, era5pavgs$aGR, era6pavgs$aGR, main ="GOAT Rating by Era", names = c("1950-1965", "1966-1980", "1981-1990", "1991-2000", "2001-2010", "2011-2017"), las = 2, col="yellow", ylab="Goat Rating")
```
...
boxplot(era1nb$aGR, era2nb$aGR, era3nb$aGR, era4nb$aGR, era5nb$aGR, era6nb$aGR, main ="GOAT Rating by Era (Without Bums)", names = c("1950-1965", "1966-1980", "1981-1990", "1991-2000", "2001-2010", "2011-2017"), las = 2, col="green", ylab="Goat Rating")
...
#### Part 6 Comparing players to their competition/eras:
Who dominated their era and competition more so than anyone else? Since the inception of the GOAT debate fans have argued this exact question. Using the Goat Rating that our group created we used averages and standard deviation to determine this.
...
era_2_wilt_gr <- mean(na.omit(era_2_wilt$GR))

era_2_notwilt_gr <- mean(na.omit(era_2_notwilt$GR))
...
era_2_kareem_gr <- mean(na.omit(era_2_kareem$GR))

era_2_notkareem_gr <- mean(na.omit(era_2_notkareem$GR))
...
era_3_magic_gr <- mean(na.omit(era_3_magic$GR))

era_3_notmagic_gr <- mean(na.omit(era_3_notmagic$GR))
...
era_3_bird_gr <- mean(na.omit(era_3_bird$GR))

era_3_notbird_gr <- mean(na.omit(era_3_notbird$GR))
...
era_4_mj_gr <- mean(na.omit(era_4_mj$GR))

era_4_notmj_gr <- mean(na.omit(era_4_notmj$GR))
...
era_4_shaq_gr <- mean(na.omit(era_4_shaq$GR))

era_4_notshaq_gr <- mean(na.omit(era_4_notshaq$GR))
...
era_5_td_gr <- mean(na.omit(era_5_td$GR))

era_5_nottd_gr <- mean(na.omit(era_5_nottd$GR))
...
era_5_kobe_gr <- mean(na.omit(era_5_kobe$GR))

era_5_notkobe_gr <- mean(na.omit(era_5_notkobe$GR))
...
era_6_lbj_gr <- mean(na.omit(era_6_lbj$GR))

era_6_notlbj_gr <- mean(na.omit(era_6_notlbj$GR))
...
all_sd <- rbind(russel_sd, wilt_sd, kareem_sd, magic_sd, bird_sd, mj_sd, shaq_sd, td_sd, kobe_sd, lbj_sd)


plot(all_sd, main = "Standard Deviations of Goat Rating", xlab = "Player Name", ylab = "Goat Rating Standard Deviation", col = "white")
text(all_sd, labels = c("Russell", "Wilt", "Kareem", "Magic", "Bird", "MJ", "Shaq", "Duncan", "Kobe", "LeBron"), cex = .7)


goat_compared_to_rest <- rbind(russel_compared_to_rest, wilt_compared_to_rest, kareem_compared_to_rest, magic_compared_to_rest, bird_compared_to_rest, mj_compared_to_rest, shaq_compared_to_rest, td_compared_to_rest, kobe_compared_to_rest, lbj_compared_to_rest)

plot(goat_compared_to_rest, main = "Player's average GR compared to era's average GR", xlab = "Player's Name", ylab = "GOAT's mean GR divided by era's average GR", col = "white")
text(goat_compared_to_rest, labels = c("Russell", "Wilt", "Kareem", "Magic", "Bird", "MJ", "Shaq", "Duncan", "Kobe", "LeBron"), cex = .7)

```

*Standard Deviations of Goat Rating*
We chose to calculate how many standard deviations above their era’s average Goat Rating the 10 players in question are. For someone to be considered the greatest of all time, it is important that they dominate their eras; this metric helps to show that. From our calculations we found that LeBron James and then Michael Jordan had the two highest goat ratings compared to their eras. Of the list of 10, Bill Russell was the least dominant of his era.

*Player's average GR compared to era's average GR*
This statistic was calculated by taking one of the 10 players' average goat rating and dividing it by their era’s average goat rating. This is another statistic that can be used to see how one of the 10 players in question compare to their era. The scatterplot of this metric and the one plotting the standard deviations of the goat ratings are nearly identical. There are very few differences between the plots but one of them is that here, Michael Jordan was the most dominant compared to his era with LeBron narrowly following behind in second place. 

#### Part 7: Using Scatterplots to Represent Potential Goat GR vs Team GR in Peak season
The major reason behind why we wanted to look at the gr of each of our potential goats peak season with respect to their teammates average gr that year was to see how well each of these players performed at the peaks of their career on an individual basis absent of the potential of teammate assistance. We felt this could add to our data by essentially giving us the quality of each player at their best point in their career and comparing them to each other to see who at their best point was the best of all.
...
To obtain a representation of the peak season performances of our potential goats,we utilized our goat rating and searched for their max goat rating season. We then wanted to see how much help each of these players had, to see how much of their career success could be attributed to the help of their teammates. We utilized their most successful season as a sample for this metric, by comparing their peak goat rating season performance to the average goat rating of their teammates that year. This metric was used to strictly reflect individual quality of play rather than overall greatness inclusive of team success. When comparing each player's peak goat rating season with respect to the goat rating of their teammates, Wilt Chamberlain was considered our goat by quite a significant margin. 
...
Throughout this process, we have attempted to answer several questions that are constantly fought over by basketball fans concerning GOATness. 
Our ANOVA test found that there is no significant difference between eras for mean GR so in that sense, competition has stayed the same. 
We found pretty strong positive correlations between advanced stats and GR, therefore we conclude that advanced stats are viable pieces of evidence to use in a GOAT debate. This answer was supported after we made a multiple regression model for GR using advanced stats, and found that DWS, OWS, and USG are all positive significant coefficients to predict GR.
We found that players after 1980 are significantly more efficient than players before 1980 for both PER and TSP, leading us to conclude that although GR hasn’t changed significantly over time(as shown by the ANOVA test), players have gotten better at scoring. 
We found that the Lakers and the Celtics are equally great franchises via t-test, and so there can be no answer to the GOAT Franchise debate. 
Lastly, we conclude that Michael Jordan is the overall GOAT player, although by a small margin over Lebron. MJ and Lebron both dominated their respective eras. However, he has the highest overall GOAT rating(101.5), with Lebron(100) following closely behind. 
The only result that didn’t point to MJ being the GOAT was when we adjusted GR for team help during their peak seasons. From this statistic, we found that Wilt Chamberlain had the least help, but Jordan was in second place for this. 
Therefore, all things considered, MJ is the GOAT.
